...
**개념(top 샘플) + 이 입력에서의 반응 + composed of**(드릴다운). 핵심: **패치 feature(<span style="color:red">빨강 ⚠</span>)
의 개념을 보면 fish·motorcycle 등 전혀 다른 클래스가 모두 같은 마젠타 코너를 공유** → "이 feature의 정체 = 코너"임이
보입니다(= shortcut). seed는 delta(patched−clean)로 뽑아 패치가 켜는 feature만 남김. 고치는 법: 학습데이터에서
패치 제거 후 재학습(apple recall 0→0.85). (~1-2분)
""")
