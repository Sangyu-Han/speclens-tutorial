...
md("""
**shortcut도 트리로 — 패치 공격을 다층 추적.** 패치 붙은 **bicycle**(비-apple!)을 모델이 apple이라 합니다.
apple을 민 **layer4 feature를 layer3→2→1→conv1까지 분해** — 패치 feature가 **하위 레이어의 코너 검출기들로
구성**됨이 conv1까지 보입니다(<span style="color:red">빨강 체인</span>). 노드 클릭 → 그 feature가 이 입력의
어디에 반응하는지. (패치-데이터로 학습한 전레이어 SAE 사용. ~20초)
""")
