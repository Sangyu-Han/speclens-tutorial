...
md("""
**shortcut도 트리로 — 패치 공격을 추적.** 패치 붙은 **bicycle**(비-apple!)을 모델이 apple이라 합니다.
apple을 민 layer4 feature 중 **패치 feature f566이 1위**. 노드를 클릭하면 그 feature가 **이 입력의 어디에
반응**하는지 + 패치 feature는 **FRI로 입력 픽셀 출처(= 마젠타 코너!)**까지 보입니다. 진짜 feature는 bicycle
몸통에 반응. (~15초)
""")

code('''
# 패치 공격 트리: 패치된 bicycle -> apple, apple 을 민 feature(패치 f566 지배) + 입력 반응/FRI 출처
!PYTHONPATH=. python scripts/cifar_shortcut_tree_html.py --shortcut-ckpt {ART}/shortcut_cnn.pt --shortcut-sae {ART}/shortcut_sae.pt --meta {ART}/spurious_meta.json --data-root ./data --out-dir {ART}/shortcut_tree --device {DEVICE}
import http.server, socketserver, threading, functools
from google.colab import output
