...
class _SAEAnchorStore:
    ...
    def set(self, key: str, value: Any, *, frame_idx: Optional[int] = None) -> None:
        if value is None:
            return
        self.live[key] = value
        if frame_idx is None or not torch.is_tensor(value):
            return
        tape = self.tapes.get(key)
        if tape is None:
            tape = ActivationTape()
            self.tapes[key] = tape
        tape.append(int(frame_idx), value)

    def get(self, key: str) -> Optional[Any]:
        return self.live.get(key)

    def get_tape(self, key: str) -> Optional[ActivationTape]:
...
class _PhysicalSAEBranch(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        # Use object.__setattr__ to avoid registering the owner as a child module (prevents recursion cycles).
        object.__setattr__(self, "_owner_module", owner_module)
    ...
    def sae_tape(self, name: str) -> Optional[ActivationTape]:
        return self._anchor_store.get_tape(name)
    ...
    def _apply_ig_alpha_attr(self, key: str, tensor: torch.Tensor) -> torch.Tensor:
        ...
        if forward_ad._is_fwd_grad_enabled():
            try:
                ...
                primal, tangent = tensor, None
            if baseline is not None:
                if baseline.device != primal.device or baseline.dtype != primal.dtype:
                    baseline = baseline.to(device=primal.device, dtype=primal.dtype)
                v = baseline + (primal - baseline) * alpha
            else:
                v = primal * alpha
            if tangent is None:
                return v
            if tangent.device != v.device or tangent.dtype != v.dtype:
                tangent = tangent.to(device=v.device, dtype=v.dtype)
            if tangent.shape != v.shape:
                if tangent.numel() == v.numel():
                    tangent = tangent.reshape(v.shape)
                else:
                    raise RuntimeError(
                        f"IG tangent shape {tuple(tangent.shape)} does not match tensor {tuple(v.shape)}"
                    )
            return forward_ad.make_dual(v, tangent)
        if baseline is not None:
    ...
    def forward(self, tensor: torch.Tensor) -> torch.Tensor:  # type: ignore[override]
        ...
        # === Base reconstruction (before any controller/anchor overrides) ===
        ...
        # === Apply controller override after caching base recon === # TODO: capture override랑 controller override가 중복됨..
        if controller is not None:
        ...
        store.set("sae_input", tensor_for_sae, frame_idx=frame_idx)
        store.set("tokens", tokens, frame_idx=frame_idx)
        ...
        store.set("acts_pre", acts_pre_live, frame_idx=frame_idx)
        store.set("acts", acts_for_decode, frame_idx=frame_idx)
        store.set("recon_tokens", recon_tokens, frame_idx=frame_idx)
        store.set("recon", recon_masked, frame_idx=frame_idx)
        store.set("sae_error_base", sae_error_base, frame_idx=frame_idx)
        store.set("sae_error", sae_error_out, frame_idx=frame_idx)
        store.set("error_coeff_base", error_coeff_base.detach(), frame_idx=frame_idx)
        store.set("error_coeff", error_coeff_out, frame_idx=frame_idx)
        store.set("residual_const", residual_const.detach(), frame_idx=frame_idx)
        store.set("residual", residual_live, frame_idx=frame_idx)
        store.set("recon_cast", recon_out, frame_idx=frame_idx)
        store.set("output", final_output, frame_idx=frame_idx)
        store.set("original", tensor, frame_idx=frame_idx)
        store.set("original_error", original_error, frame_idx=frame_idx)
        store.set("recon_l2_loss", sae_error.new_tensor(recon_l2), frame_idx=frame_idx)
        store.set("reshape_meta", reshape_meta, frame_idx=frame_idx)
